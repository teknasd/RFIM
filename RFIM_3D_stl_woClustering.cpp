...
#define VER		    5	     //  width of lattice matrix of edge 6 and 6*6*6 lattice
...
#define N			VER*VER*VER  // width of edge matrix. for ex 6*6*6 edges
...
{
	string file_name = "magnetization";
	file_name += to_string(VER) + ".txt";
...
void copy_to_file(ofstream & ifile, int delta ,int mag_val)
{
...
void create_Bmat(vector <int>& Bmat)
{
...
int magnetization(vector <int> & M,int size)
{
	long mag_value=0,i;
	for ( i = 0; i < size; i++)
	{
	...
	mag_value = abs(mag_value);
	cout<<"magnetization value:"<<mag_value  <<endl;
	return mag_value;
}
...
int main(void) {
	...
	string file_name = "magnetization/mag_values_";
	file_name += to_string(VER) + ".txt";
	ofstream input_file(file_name);
	//input_file = create_file();
	time_t time_begin, time_end, time_1, time_2;

	long t1, t2, tdiff[iter] = { 0 }, l = 0, sum = 0, i, j, cap = 0,k,mag_value;
	for (l = 0; l < iter; l++)
	{
		cout << "\n\n*****ITER:  " << l << "****\n\n";
		...
		*/
		...
		for (i = 0; i < N; i++)
		{
			if (((i + 1) % VER != 0) )//not on right vertical edge
			{
				Exmat[i][i + 1] = 1 * latt[i] * latt[i + 1];
				Exmat[i+1][i] = 1 * latt[i] * latt[i + 1];
				//cout << Exmat[i][i + 1];
			}
			if (i+VER*VER < N)
			{
				Exmat[i][i + VER*VER] = 1 * latt[i] * latt[i + VER*VER];
				Exmat[i + VER*VER][i] = 1 * latt[i] * latt[i + VER*VER];
			...
			if (i + VER < N)
			{
				...
				Exmat[i + VER][i] = 1 * latt[i] * latt[i + VER];
			}
			//else 
		}
		...
		for (del = del_beg; del <= del_end; del += del_inc)
		{
			...
			mag_value = magnetization(visited, V);
			copy_to_file(input_file, del, mag_value);
			/*if (input_file.is_open())
			{
				input_file << del;
				input_file << d;
				input_file << mag_value;
				input_file << endl;
			}*/
			//its clustering time
